...
datasets = []
n = 0
for i in range(len(ds.lat[:,0])):
    for j in range(len(ds.lon[0,:])):
        # print(i,j)
        if [i,j] in tri_idx:
            # print(i,j)
            single_point =  ds.bnum[idxTimei:idxTimef,:,0,i,j]
...
p_mod = ax1.plot(MOD_bnum_3nm_tot.index,MOD_bnum_3nm_tot,label='MODEL')
...
plt.savefig('output_figures/bnum_timeseries_TRIANG.png')
# plt.show()
...
ax1.plot(MOD_bnum_tot_D.index,MOD_bnum_tot_D,label='MODEL')
...
plt.savefig('output_figures/bnum_diurnal_TRIANG.png')
# plt.show()
...
ax1.loglog(center_diameters_3nm*1e9,1*PYR_dNdlogDp_mean_3nm.flatten(),label='Model') # pyr
...
plt.savefig('output_figures/bnum_sizedist_TRIANG.png')
# plt.show()
