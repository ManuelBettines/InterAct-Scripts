...
tem2_data = ds['tem2'].sel(x=idx_lon).sel(y=idx_lat) - 273.15
...
temp = observational_data_t['HYY_META.T42']
...
diurnal_cycle_observation = observational_data_t.groupby('hour')['HYY_META.T42'].mean()
std_observation = observational_data_t.groupby('hour')['HYY_META.T42'].std()
...
#ax.set_ylim([0,1])
ax.set_xlim([0,23])
fig.savefig("../figures/daily_temperature.png", dpi=350, bbox_inches='tight')
plt.show()
