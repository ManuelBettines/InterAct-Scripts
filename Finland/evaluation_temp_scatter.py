...
ds['tem2'] = ds.tem2.swap_dims({'time_counter':'local_times'})
ds = ds.assign_coords(hour=('local_times', local_times.hour))

model = ds['tem2'].sel(x=idx_lon).sel(y=idx_lat) - 273.15
...
observational_data_t.set_index('datetime', inplace=True)
observational_temps = observational_data_t['HYY_META.T42'].resample('H').mean()
...
plt.text(0.05, 0.95, f"MBE: {mbe:.2f} °C", ha='left', va='top', transform=plt.gca().transAxes, fontsize=9, bbox=dict(boxstyle="round", facecolor="white", alpha=0.8))
#y0 = 2*r0
#y1 = 0.5*r0
#ax.plot(r0,y0,'k--')
#ax.plot(r0,y1,'k--')
...
ax.set_ylim([-2,32])
ax.set_xlim([-2,32])
plt.xticks([0,10,20,30], [0,10,20,30])  # Custom x-ticks
plt.yticks([0,10,20,30], [0,10,20,30])
plt.gca().set_aspect('equal', adjustable='box')
fig.savefig("../figures/temperature_scatter.png", dpi=350, bbox_inches='tight')
plt.show()
