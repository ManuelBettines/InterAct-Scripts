...
# Load model output
ds = xr.open_dataset("../data/meteo_FINLAND6_CC.nc")
...
tem2_data = (np.arctan2(-winz, -winm) * 180 / np.pi) % 360
...
temp = observational_data_t['HYY_META.WDU336']
...
def circular_std(degrees):
    radians = np.deg2rad(degrees)
    mean_cos = np.mean(np.cos(radians))
    mean_sin = np.mean(np.sin(radians))
    R = np.sqrt(mean_cos**2 + mean_sin**2)
    if R > 0:  # Avoid log of zero
        return np.sqrt(-2 * np.log(R))
    else:
        return np.nan 

diurnal_cycle_model = tem2_data.groupby('hour').apply(circular_mean)
std_model = tem2_data.groupby('hour').std()#.apply(circular_std)

observational_data_t['hour'] = observational_data_t['datetime'].dt.hour 
diurnal_cycle_observation = observational_data_t.groupby('hour')['HYY_META.WDU336'].apply(circular_mean)
std_observation = observational_data_t.groupby('hour')['HYY_META.WDU336'].std()#.apply(circular_std)

print(std_observation)
...
ax.set_ylim([0,360])
ax.set_xlim([0,23])
fig.savefig("../figures/daily_wind_direction.png", dpi=350, bbox_inches='tight')
plt.show()
